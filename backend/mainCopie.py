...
def clean_regions(regions: List[int]) -> List[int]:
    """Exclut BEN_REG=0 et BEN_REG=99 (région inconnue) de tout filtre régional."""
    return [r for r in regions if r not in (0, 99)]
...
def get_db():
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Base de données indisponible : {e}")
